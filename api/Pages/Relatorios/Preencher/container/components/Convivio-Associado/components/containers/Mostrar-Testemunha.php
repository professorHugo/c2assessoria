<?php
  if( $SysMode == 1 ){
    ...
    while( $Testemunha = mysqli_fetch_assoc($ExeQrBuscarTestemunhas) ){
      ...
      <form 
        action="
          ?Page=Relatorios&Preencher&Protocolo=<?php echo $Protocolo?>&Content=Convivio-Associado&SalvarEntrevistado=<?php echo $IdEntrevistado?>" 
        ...
        >
          ...
          <?php
            if( $TipoEntrevistado == 2 ){
              ?>
                <input type="hidden" name="Sabedor-Convivio">
              <?php
            }
          ?>
